...
export async function GET() {
  ...
  try { const data = await getSyncedData(); return NextResponse.json({ source: data.source, count: data.counts.insumos, items: data.insumos }, { headers: { "Cache-Control": "no-store" } }); } catch { return NextResponse.json({ code: "DATABASE_UNAVAILABLE" }, { status: 503 }); }
}
